...
const FUEL_LABEL: Record<FuelKey, string> = {
  gazole: 'Gasolio (B7 diesel)',
  sp95: 'Benzina',
  sp98: 'Benzina 98 / Plus',
  e10: 'Benzina E10',
  e85: 'Bioetanolo',
  gplc: 'GPL',
};
...
export default function ItalyRegionalView({ data }: Props) {
  const [selectedRegion, setSelectedRegion] = useState<string>('IT');
  ...
  const view = useMemo(() => {
    ...
    if (selectedRegion !== 'IT' && data.regions[selectedRegion]) return data.regions[selectedRegion];
    return {
      name: 'All Italy',
  ...
  return (
    <section aria-label="Live station prices — Spain" className="rounded-lg border border-oil-800 bg-oil-900/30 overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-oil-800/40 bg-oil-950/40">
        <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2">Regione</div>
        ...
        {selectedRegion !== 'IT' && provsForSelectedRegion.length > 0 && (
          <div className="mt-3">
      ...
      <div className="px-5 py-4">
        ...
        {selectedProv && (
          <a
            href={`/country/it/prov/${selectedProv.toLowerCase()}`}
      ...
      <div className="px-5 py-2 border-t border-oil-800/40 bg-oil-950/40">
        <p className="text-[10px] text-gray-500 leading-snug">
          ...
          (Ministero delle Imprese e del Made in Italy). Refreshed daily. Methodology:{' '}
