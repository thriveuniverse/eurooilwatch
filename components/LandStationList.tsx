...
export default function ProvStationList({ stations, landName, initialCity = '' }: Props) {
  ...
  const scopeLabel = filterActive ? `"${filter}" in ${landName}` : landName;
    ...
    <>
      ...
      {filteredStations.length === 0 ? (
        <section className="rounded-lg border border-oil-800 bg-oil-900/30 px-5 py-8 text-center text-sm text-gray-400">
          No stations match <strong className="text-white">{filter}</strong> in {landName}. Try a different spelling, or{' '}
        ...
        <>
          <section className="space-y-4">
            <h2 className="text-2xl font-bold text-white">
              Cheapest stations {filterActive ? 'matching your search' : `in ${landName}`}
            </h2>
          ...
          <section className="space-y-3">
            <h2 className="text-2xl font-bold text-white">
              All stations {filterActive ? `matching "${filter}"` : `in ${landName}`}
            </h2>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-oil-900/50">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-amber-400/80 text-right">Diesel</th>
                    <th className="px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-amber-400/80 text-right">Super E5</th>
                    <th className="px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-amber-400/80 text-right">Super Plus</th>
                  </tr>
                </thead>
                <tbody>
