...
const FUEL_LABEL: Record<FuelKey, string> = {
  gazole: 'Gasóleo simples (diesel)',
  sp95: 'Gasolina simples 95',
};

const FUELS: FuelKey[] = ['gazole', 'sp95'];

function fmtPrice(v: number | undefined): string {
  return v === undefined ? '—' : `€${v.toFixed(3)}`;
}

function deltaVsNational(area: number | undefined, nat: number | undefined): { className: string; label: string } | null {
...
export default function PortugalRegionalView({ data }: Props) {
  const [selectedRegion, setSelectedRegion] = useState<string>('PT');
  const [selectedProv, setSelectedProv] = useState<string>('');

  const nationalFuels = data.national.fuels;

  const regionList = useMemo(
    () => Object.entries(data.regions).map(([code, r]) => ({ code, ...r })).sort((a, b) => a.name.localeCompare(b.name)),
    [data.regions]
  );

  const provsForSelectedRegion = useMemo(() => {
    if (selectedRegion === 'PT') return [];
  ...
  const view = useMemo(() => {
    ...
    if (selectedRegion !== 'PT' && data.regions[selectedRegion]) return data.regions[selectedRegion];
    return { name: 'All Portugal', stationCount: data.national.stationCount, fuels: nationalFuels } as AreaStats;
  }, [data, selectedRegion, selectedProv, nationalFuels]);

  const updatedDate = new Date(data.asOf).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });

  return (
    <section aria-label="Live station prices — Portugal" className="rounded-lg border border-oil-800 bg-oil-900/30 overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-oil-800/40 bg-oil-950/40">
        <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2">Região</div>
        <div className="flex flex-wrap gap-1.5">
          <button
            onClick={() => { setSelectedRegion('PT'); setSelectedProv(''); }}
          ...
          {regionList.map((r) => (
            <button
              key={r.code}
              onClick={() => { setSelectedRegion(r.code); setSelectedProv(''); }}
              className={`text-[11px] px-2.5 py-1 rounded transition ${
        ...
        {selectedRegion !== 'PT' && provsForSelectedRegion.length > 0 && (
          <div className="mt-3">
            <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2">
              Distrito (within {data.regions[selectedRegion]?.name})
            </div>
            <select
              value={selectedProv}
      ...
      <div className="px-5 py-4">
        <div className="flex items-baseline justify-between flex-wrap gap-2 mb-3">
          ...
          <span className="text-xs text-gray-500">
            ...
            {view.provinceCount ? ` · ${view.provinceCount} distritos` : ''}
          </span>
        </div>

        {selectedProv && (
          <a
            href={`/country/pt/distrito/${selectedProv.toLowerCase()}`}
      ...
      <div className="px-5 py-2 border-t border-oil-800/40 bg-oil-950/40">
        <p className="text-[10px] text-gray-500 leading-snug">
          ...
          (Direção-Geral de Energia e Geologia). Free, non-commercial use. Refreshed daily. Methodology:{' '}
