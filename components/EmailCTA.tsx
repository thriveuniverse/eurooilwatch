...
export default function EmailCTA() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    // For now, open mailto with the signup request
    // Replace with Resend/Make webhook when ready
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1">
          <h2 className="text-sm font-semibold text-white">
            Weekly EU Fuel Security Briefing
          </h2>
          <p className="mt-1 text-xs text-gray-400">
            Get a concise weekly summary of reserve levels, price movements,
            and supply risks — delivered every Thursday.
          </p>
        </div>
        {submitted ? (
