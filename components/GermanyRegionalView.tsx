...
export default function GermanyRegionalView({ data }: Props) {
  ...
  const view = useMemo(() => {
    ...
    if (selectedRegion !== 'DE' && data.regions[selectedRegion]) return data.regions[selectedRegion];
    return {
      name: 'All Germany',
  ...
  return (
    <section aria-label="Live station prices — Spain" className="rounded-lg border border-oil-800 bg-oil-900/30 overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-oil-800/40 bg-oil-950/40">
        <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2">Bundesland</div>
        ...
        {selectedRegion !== 'DE' && provsForSelectedRegion.length > 0 && (
          <div className="mt-3">
            <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2">
              Bundesland (within {data.regions[selectedRegion]?.name})
            </div>
            <select
              value={selectedProv}
      ...
      <div className="px-5 py-4">
        ...
        {selectedProv && (
          <a
            href={`/country/de/land/${selectedProv.toLowerCase()}`}
      ...
      <div className="px-5 py-2 border-t border-oil-800/40 bg-oil-950/40">
        <p className="text-[10px] text-gray-500 leading-snug">
          ...
          <a href={data.source} target="_blank" rel="noopener noreferrer" className="text-oil-400 hover:underline">
            Tankerkönig (MTS-K mirror)
          </a>{' '}
          (Bundeskartellamt-licenced, CC BY 4.0). Refreshed daily. Methodology:{' '}
