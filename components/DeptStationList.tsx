...
export default function DeptStationList({ stations, deptName, initialCity = '' }: Props) {
  ...
  const scopeLabel = filterActive ? `"${filter}" in ${deptName}` : deptName;

  return (
    <>
      {/* Filter input */}
      <section className="rounded-lg border border-amber-700/40 bg-amber-950/10 px-5 py-4">
        <label htmlFor="dept-filter" className="block text-[10px] font-mono uppercase tracking-wider text-amber-400/80 mb-2">
          Filter by city, address, or postal code
        </label>
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            id="dept-filter"
            type="text"
            placeholder="e.g. Toulouse, 31000, Avenue de Lyon"
            value={filter}
      ...
      {filteredStations.length === 0 ? (
        <section className="rounded-lg border border-oil-800 bg-oil-900/30 px-5 py-8 text-center text-sm text-gray-400">
          No stations match <strong className="text-white">{filter}</strong> in {deptName}. Try a different spelling, or{' '}
        ...
        <>
          {/* Cheapest stations per fuel — recomputed from filter */}
          <section className="space-y-4">
            <h2 className="text-2xl font-bold text-white">
              Cheapest stations {filterActive ? 'matching your search' : `in ${deptName}`}
            </h2>
          ...
          <section className="space-y-3">
            <h2 className="text-2xl font-bold text-white">
              All stations {filterActive ? `matching "${filter}"` : `in ${deptName}`}
            </h2>
